...
// x = 200
int is_declare_statement(struct Token tokens[], unsigned long ntokens) {
    return
    tokens[0].token == WORD
    && tokens[1].token == EQUAL
    && tokens[ntokens].token == SEMICOLON;
}

int construct_declare_statement(char *line, struct Token tokens[], unsigned long ntokens, struct Statement *stmt) {
    ...
    construct_expr(line, tokens+2, ntokens, expr);
...
int construct_statement(char *line, struct Token tokens[], long unsigned nstmt, struct BlockStatement *block) {
    ...
    // declare var
        ...
        construct_declare_statement(line, tokens, nstmt, &new);
...
int parse(char *filepath) {
    ...
    while (n > 0) {
        ...
        unsigned long ntokens = tokenize(line, tokens, token_n);
