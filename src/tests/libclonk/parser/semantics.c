...
{
    onk_semantic_build_frame(parser);
...
void setup(struct onk_parser_input_t *input, struct onk_token_t *tokens, uint16_t tokens_sz)
{
    uint16_t i = 0;

    input->add_glob_scope = true;

    onk_vec_init(&input->tokens);
    onk_vec_alloc_stk(&input->tokens, tokens, tokens_sz, sizeof(struct onk_token_t));
}
...
// while(
//      ^
void __test__semantic_param_after_while(CuTest* tc)
{
    ...
    mock_current(&parser, ONK_WHILE_TOKEN);
    compile(&frame, &parser);

    CuAssertTrue(tc, parser.nexpect == 1);
    CuAssertTrue(tc, parser.expect[0] == ONK_PARAM_OPEN_TOKEN);
}

// if(
//   ^
/* void __test__semantic_param_after_if(CuTest* tc) */
/* { */
/*     struct validator_frame_t frame; */
/*     mock_current(parser, ONK_IF_TOKEN); */
/*     compile(&frame, parser); */
...
/* // for( */
/* //    ^ */
/* void __test__semantic_param_after_for(CuTest* tc) */
/* { */
/*     struct validator_frame_t frame; */
/*     mock_current(&state->parser, ONK_FOR_TOKEN); */

/*     compile(&frame, &state->parser); */
...
// word.word
//     ^^
/* void __test__semantic_word_after_dot(CuTest* tc) */
/* { */
/*     struct validator_frame_t frame; */
...
/* // struct word */
/* //      ^-^ */
/* void __test__semantic_word_after_struct(CuTest* tc) */
/* { */
/*     struct validator_frame_t frame; */
...
// unit +
//    ^-^
void __test__semantic_operator_after_unit(CuTest* tc)
{
    struct validator_frame_t frame;
    char buf[254];
...
// .. + unit
//    ^-^
/* void __test__semantic_unit_after_operator(CuTest* tc) */
/* { */
/*     const uint8_t answer_len = UNIT_LEN + UNIOP_LEN + 3; */
...
/*        Group delimiter       */
/********************************/
/* void __test__delim_list(CuTest* tc) */
/* { */
/*     struct validator_frame_t frame; */
...
/*     mock_current(&state->parser, ONK_WORD_TOKEN); */
/*     onk_semantic_build_frame(&state->parser); */
